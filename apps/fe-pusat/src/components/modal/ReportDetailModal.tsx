...
import { WEBDDS_TOPICS, publish } from "../../utils/api/disaster.webdds";
...
const ReportDetailModal = memo(
	({ report, onClose }: ReportDetailModalProps) => {
		const updatePenanganan = useDisasterStore((s) => s.updatePenanganan);

		const [confirmPendingStatus, setConfirmPendingStatus] = useState<
			number | null
		>(null);
		...
		const executeUpdateStatus = useCallback(() => {
			if (confirmPendingStatus !== null) {
				...
				let messageStr = "";
				let logType: "danger" | "warning" | "success" | "neutral" | "system" =
					"neutral";
				if (
					confirmPendingStatus === enumMap.statusPenanganan.STATUS_SEDANG_PROSES
				) {
					messageStr = `Laporan sedang diproses: ${report.sourceName}`;
					logType = "warning";
				} else if (
					confirmPendingStatus === enumMap.statusPenanganan.STATUS_SUDAH_DIATASI
				) {
		...
		if (confirmPendingStatus !== null) {
			...
			let btnColor: "error" | "warning" | "success" | "primary" | "neutral" =
				"error";
				...
				>
					<div className="text-center pt-2">
						...
						<div className="bg-neutral-50 border border-neutral-100 px-6 py-5 mb-6 text-sm flex flex-col gap-5 relative opacity-90 rounded">
							<div>
								<span className="text-[9px] text-gray-400 uppercase tracking-[0.15em] block font-bold mb-1.5">
									Lokasi
								</span>
								<span className="font-bold text-surface-dark text-[15px]">
									{report.sourceName}
								</span>
							</div>
						...
						<p className="text-[10px] text-left text-gray-500 italic flex items-start gap-2 mb-8 leading-relaxed">
							<span className="bg-gray-400 text-white rounded-full w-4 h-4 flex items-center justify-center shrink-0 not-italic text-[10px] uppercase font-bold mt-0.5">
								!
							</span>
							Data yang dikirimkan akan segera didistribusikan ke unit tanggap
							darurat terkait. Pastikan seluruh informasi sudah akurat.
						</p>
						...
						<div className="flex flex-col gap-3">
							<Button
								...
								className="uppercase font-bold tracking-widest py-4 hover:brightness-90"
							>
								Konfirmasi Update Status{" "}
								<span className="ml-[2px] tracking-normal mb-[1px]">➤</span>
							</Button>
							<Button
								variant="outline"
								color="neutral"
								fullWidth
								onClick={() => setConfirmPendingStatus(null)}
								className="uppercase font-bold tracking-widest py-4 border-gray-200 text-gray-400 hover:text-gray-600"
							>
								Batal
							</Button>
						</div>
				...
				<div className="flex flex-col gap-7 pt-2">
					...
					<div>
						...
						<div className="flex justify-between py-3 border-b border-gray-100 text-[13.5px]">
							<span className="text-gray-500">Jenis Bencana</span>
